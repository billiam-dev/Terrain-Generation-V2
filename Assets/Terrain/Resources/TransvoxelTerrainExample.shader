    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 triplanarW = ComputeTriplanarWeights(normalize(IN.normalWS));
                GetTriplanarCoordinate(IN.positionWS, uv0, uv1, uv2);
                
                a1 = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, uv0 * _BaseMap_ST.xy + _BaseMap_ST.zw) * triplanarW.y;
                a2 = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, uv1 * _BaseMap_ST.xy + _BaseMap_ST.zw) * triplanarW.z;
                a3 = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, uv2 * _BaseMap_ST.xy + _BaseMap_ST.zw) * triplanarW.x;

                half4 albedoColor = (a1 + a2 + a3) * _BaseColor;

                // Compute light color.
                Light mainLight = GetMainLight();
                half lightIntensity = saturate(dot(IN.normalWS, mainLight.direction));
                half3 lightColor = mainLight.color * lightIntensity + _GlossyEnvironmentColor.rgb;

                return albedoColor * half4(lightColor, 1.0h) * (_ClipmapDebugColor + 0.5h / 2.0h);
            }
